...
class mail_alias(osv.Model):
    ...
    _columns = {
        'alias_name': fields.char('Alias Name',
            help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.modoo.com>",),
        'alias_model_id': fields.many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                          help="The model (Modoo Document Kind) to which this alias "
    ...
    def name_get(self, cr, uid, ids, context=None):
        """Return the mail alias display alias_name, including the implicit
           ...
           e.g. `jobs@mail.modoo.com` or `jobs` or 'New Alias'
        """
        res = []
        for record in self.browse(cr, uid, ids, context=context):
