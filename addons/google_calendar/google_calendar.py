    ...
    def compute_OP(self, modeFull=True):
        ...
        # New in openERP...  Create on create_events of synchronize function
            ...
            if self.OE.status:
                ...
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from Modoo, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in Modoo")
...
all links between GoogleID and Modoo Google Internal ID', copy=False)
    }
...
class calendar_event(osv.Model):
    ...
    _columns = {
        'oe_update_date': fields.datetime('Modoo Update Date'),
    }
...
class calendar_attendee(osv.Model):
    ...
    _columns = {
        ...
        'oe_synchro_date': fields.datetime('Modoo Synchro Date'),
    }
