...
Modoo celebrates freedom. Freedom for the designer to go further and
freedom for the user to customize everything according to their needs.

Ready to create your own theme? Great. Here are some things you should know before you begin. This tutorial is a guide to creating an Modoo theme.

.. image:: theme_tutorial_assets/img/Intro.jpg

.. warning::

  This tutorial requires having installed Modoo v8.0 and the Theme Support Engine (website_less) module. https://www.modoo.com/apps/8.0/website_less/


An introduction for web designers
=================================

If you are a web designer using Modoo for the first time, you are in the right place.
This introduction will outline the basics of Modoo theme creation.

.. note::

   Modoo’s team has created a framework that’s powerful and easy to use. There’s no need to know special syntaxes to use this set of tools.

From common CMS to Modoo
-----------------------
...
This is usually the first question from a web designer used to working with Wordpress or Joomla and coming to Modoo for the first time.
...
.. container:: col-sm-7
  ...
  This **does not** apply to creating Modoo themes.
...
Modoo default theme structure
----------------------------

.. container:: col-sm-8

  Modoo comes with a default theme structure.
  ...
  It will upgrade automatically within your Modoo installation and, since it is included in the Website Builder module, everything is smoothly integrated by default.
...
An Modoo theme is not a folder containing HTML or PHP files, it’s a modular framework written in XML. Never worked with XML files before? Don’t worry, after following the tutorial, you’ll be able to create your first theme with only basic knowledge of HTML.
...
Creating themes for Modoo is a total change of perspective. Instead of defining the complete layout for a page, you can create blocks (snippets) at let the user choose where to “drag&drop” them, creating the page layout on their own.
We call this modular design.

Imagine an Modoo theme as a “list” of elements and options that you have to create and style.
...
.. figure:: theme_tutorial_assets/img/functionalities.jpg
   ...
   Thanks to Modoo’s modularity, everything can be personalized even more. This means there are endless possibilities for your creativity. Adding functionalities is easy and it’s simple to provide the end user with customizable options.

.. /row

Modoo's XML files, an overview
-----------------------------

Any Modoo XML file starts with encoding specifications.
...
the **Modoo default structure**.  In order to do that you can use
...
Modoo’s themes are packaged like modules. Even if you are designing a very simple website for your company or client, you need to package the theme like an Modoo module.
...
These values will be used to identify your new theme in Modoo’s backend.
...
``category`` defines your module category (always “Theme”) and, after a slash, the subcategory. You can use one subcategory from the Modoo Apps categories list. (https://www.modoo.com/apps/themes)
...
To install your theme, you just place your theme folder inside addons in your Modoo installation.

After that, navigate to the Settings page, look for your theme and click on the install button.

Structure of an Modoo page
=========================

An Modoo page is the visual result of a combination of 2 kind of elements, **cross-pages** and **unique**.
By default, Modoo provides you with a **Header** and a **Footer** (cross-pages) and a unique main element that contains the content that makes your page unique.
...
By default, Modoo header contains a responsive navigation menu and the company’s logo. You can easily add new elements or style the existing one.

To do so, create a **layout.xml** file in your **views** folder and add the default Modoo xml markup.
...
.. warning::
  ...
  your changes will take priority in any future Modoo’s update.
...
default Modoo markup.  Inside ``<data>`` create a ``<template>`` tag, set the
...
Using ``<t t-call="website.layout">`` we will extend the Modoo
...
Modoo includes Bootstrap by default. This means that you can take advantage of all Bootstrap styles and layout functionalities out of the box.
...
Let’s navigate to the view folder and create an XML file called *assets.xml*. Add the default Modoo xml markup and copy/paste the following code. Remember to replace ``theme folder`` with your theme’s main folder name.
...
attribute tells Modoo that our template is referring to another one in
order to operate.
...
Our less file is now included in our theme, it will be automatically compiled, minified and combined with all Modoo’s assets.
...
Add the default Modoo xml markup and copy/paste the following code.
...
We start by adding a new file in our views folder - name it **options.xml** and add the default Modoo XML markup. Create a new template copy/pasting the following
