...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects Modoo:
    ...
    ``states``
        ...
        listed states
    ``context``
        merged into the view's context when performing the button's Modoo call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's Modoo call
...
Semantic components tie into and allow interaction with the Modoo
system. Available semantic components are:

``button``
  call into the Modoo system, similar to :ref:`list view buttons
...
``templates``
  ...
  ``record``
    ...
    <https://github.com/modoo/modoo/blob/8.0/addons/web_kanban/static/src/js/kanban.js#L900>`_)
  ``read_only_mode``
    self-explanatory
...
``field``
    ...
    ``widget``
        ...
        standard Modoo 8.0 is a ``selection`` widget for
...
``filter``
    ...
    ``domain``
        an Modoo :ref:`domain <reference/orm/domains>`, will be appended to the
